...
def loginUser(request):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'Multiple accounts use this email. Please contact the administrator.')
            return redirect('login')
        ...
        if user.is_restaurant:
            messages.error(request, 'This account is registered as a restaurant.')
            return redirect('login')
...
def registerUser(request):
    if request.method == 'POST':
        ...
        if len(name) < 2:
            messages.error(request, 'Name must contain at least 2 characters.')
            return redirect('register')
...
def add_review(request, food_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('add_review', food_id=food.id)
